...
// -------------------------
// 맵드 타입(Mapped Type)
// 기존의 타입을 기반으로 새로운 타입ㅇ르 일괄 생성할 때 사용
type User3 = {
  id: number;
  name: string;
  age: number;
...
// 모든 속성을 readonly로 바꾸고 샆다.
type readonlyUser3 = {
  readonly [K in keyof User3]: User3[K];
}

// -------------------------
// 템플릿 리터널 타입(Template Literal Types)
// string leteral 타입해서 새로운 string literal 타입을 생성
type Color2 = 'red' | 'green' | 'blue';
type Intensity = 'light' | 'dark';
type ColorTheme = `${Intensity}-${Color2}`; // 프런트에서 많이 씀(ex.클래스명)